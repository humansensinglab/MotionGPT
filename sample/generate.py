...
def main():
    ...
    if args.text_prompt != '':
        ...
        for entry in entries:
            with open('dataset/HumanML3D/texts/{}.txt'.format(entry), 'r') as fr:
                text = fr.readline()
                text = text.split('#')[0]
            texts.append(text)
            with open('dataset/HumanML3D/babel_texts/{}.txt'.format(entry), 'r') as fr:
                babel_text = fr.readline()
            babel_texts.append(babel_text)

        for i in range(len(texts)):
            if babel_texts[i] == 'None':
                babel_texts[i] = texts[i]

        args.num_samples = len(texts)

        # assert os.path.exists(args.input_babel_text)
        # with open(args.input_babel_text, 'r') as fr:
        #     babel_texts = fr.readlines()
        # babel_texts = [s.replace('\n', '') for s in babel_texts]
    ...
    if is_using_data:
        ...
        if is_t2m:
            # t2m
            collate_args = [dict(arg, text=txt, babel_text=babel_txt) for arg, txt, babel_txt in zip(collate_args, texts, babel_texts)]
        else:
            # a2m
